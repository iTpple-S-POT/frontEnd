                ...
                ForEach(viewModel.userInterestMatrix, id: \.self) { list1D in
                    ...
                    HStack(spacing: 9) {
                        ...
                        ForEach(list1D) { element in
                            ...
                            SpotStateButton(innerView: AnyView(Text(element.rawValue).font(.suite(type: .SUITE_Regular, size: 16))), idleColor: .spotLightGray, activeColor: .spotRed, frame: viewModel.getViewSize(string: element.rawValue)) {
